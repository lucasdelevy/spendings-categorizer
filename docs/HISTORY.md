...
Key changes:
- New shared helper `isRefund` / `normalizeRefundSign` (one copy in `src/engine/refunds.ts` for the frontend CSV parsers, one in `backend/src/services/refunds.ts` for the Pierre/Open Finance path). The helper does a case-insensitive substring match against a curated keyword list and forces matching rows to `Math.abs(amount)`.
- `cardCategorizer.ts`: after the standard sign flip (charges are exported positive, payments/refunds negative), refund rows are forced positive so they cannot end up negative regardless of how the source CSV chose to encode them.
- `bankCategorizer.ts`: applies the same refund-sign normalization to the parsed `Valor` column, defending against bank exports that occasionally publish estornos with the same sign as charges.
- `pierreService.ts` (`mapPierreTransaction`): credit-card-account transactions are now negated as a class (Pluggy convention: positive = charge, negative = payment/refund), so refunds end up positive and charges end up negative. Bank-account logic is unchanged. After the per-account-type normalization, refunds are unconditionally `Math.abs`-clamped to handle any remaining edge cases where the upstream connector classifies an estorno as `type: "DEBIT"` with a positive amount.
- `familyCategorizer.ts`: removed the `t.amount < 0` filter on card transactions, which was silently dropping legitimate (positive) card refunds from the family/dashboard view. Card transactions are now passed through untouched and refunds correctly offset their original charges in category totals.
- `categoryService.applyCategoryConfig`: every time a month is opened the backend re-runs the keyword pass and, in the same loop, heals any historic refund row whose `amount` is negative by flipping it positive and recomputing the statement summary (`totalIn`, `totalOut`, `balance`, per-category totals). This means data ingested before the fix self-repairs on first view without needing a one-shot migration.
