...
- New "Contas e Cartões" sidebar entry and `AccountsPage` in the frontend (`useAccounts` hook) with full CRUD: name, type toggle, closing day input, masked API key field with "Remove key" action and a "leave empty to keep current" placeholder when editing.
- `SaveConfirmBar` lets the user assign each uploaded CSV to one of their accounts before saving (auto-selects when only one matching account exists, shows "No bank/card account configured" when none match). The saved transactions then carry the `accountId` so card billing-month bucketing works on subsequent reads.
- `TransactionTable` shows a small uppercase chip with the account name next to each transaction's payee.
- `POST /statements/{id}/assign-account` endpoint and a new "Account" column on the Manage Months page let users retroactively (re)assign a saved statement to an account. The handler re-tags every transaction with the new `accountId`, recomputes per-tx `billingMonth` against the new card's closing day, and splits the record across statement months when needed (any transactions whose computed `billingMonth` differs from the record's month are migrated into a new statement record under the right month and removed from the original; the original is soft-deleted if it ends up empty).
- The list endpoint (`GET /statements`) now exposes `accountId` (single value when all txs share one) and `mixedAccounts` so the Manage Months UI can show the current assignment and warn before overwriting heterogeneous data.
...
## Phase 15: Pierre Scheduled Sync Sourced from Accounts Table

Removed the `PIERRE_USER_ID` and `PIERRE_API_KEY` Lambda env vars, which were the last hard-coded coupling between the Pierre integration and a single operator's Google ID. The 5‑minute scheduled sync now discovers operators directly from the accounts table — the same single source of truth the API keys themselves live in — so onboarding a new Pierre user is purely a Manage Accounts UI action with no infrastructure redeploy.

Key changes:
- New `accountService.listOwnersWithApiKeys()` does a paginated `Scan` filtered by `begins_with(SK, "ACCT#") AND attribute_exists(apiKeyEncrypted)` and returns the distinct set of owning `USER#…` and `FAMILY#…` partition keys.
- Pierre scheduled handler (`handleScheduled`) now iterates that owner list. Personal owners use the user's own id; family-owned keys resolve to the family's owner member (or the first active member as a fallback) so statement attribution stays stable. Each owner runs through `syncMonths` independently and failures are isolated per owner.
- Manual sync (`POST /pierre/sync`) drops the `PIERRE_USER_ID` operator gate. Authorization is now uniformly "you must own at least one account with an Open Finance API key", matching how account API keys are already scoped.
- The legacy `PIERRE_API_KEY` env-var fallback is removed from `syncMonths` and the manual-update path. Per-account keys (encrypted in DDB) are now the only sync source.
- CDK stack stops wiring `PIERRE_API_KEY` and `PIERRE_USER_ID` into the Pierre Lambda. This eliminates the "scheduled sync silently no-ops after a redeploy that forgot to export the env var" failure mode that broke May 2026 imports.
